...
### 📸 이미지 업로드 설정 (ImgBB 사용 - 가장 간단!)

사용자가 **무료 이미지 호스팅 서비스**를 통해 쉽게 사진을 올릴 수 있습니다.

#### 구글 폼 설정
1. 물품 등록용 폼에서 **사진 URL** 질문 추가 (단답형)
2. 질문 설명에 아래 안내문구 복사해서 붙여넣기:
...
2. "이미지 업로드" 클릭
3. 사진 선택 후 업로드
4. "직접 링크" 복사해서 여기에 붙여넣기
(선택사항 - 사진 없어도 등록 가능합니다)
```

3. **필수 여부 끄기** (선택 사항으로 설정)

> 💡 **ImgBB란?** 무료 이미지 호스팅 서비스입니다. 회원가입 없이 바로 사용 가능!

#### 스프레드시트 헤더 설정
폼 응답 시트의 헤더에 다음 열이 있는지 확인하세요:
- `물품명` 또는 `Title`
- `카테고리` 또는 `Category`  
- `설명` 또는 `Description`
- `사진 URL` 또는 `ImageURL`

> ⚠️ 앱에서 이미지가 표시되려면 헤더 이름이 위와 일치해야 합니다!
